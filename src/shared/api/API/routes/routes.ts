...
export const getRoutes = async (from: number, to: number, date: Date, person: number): Promise<RouteType[]> => {
    const response = await api.get(`/search?departure=${from}&arrival=${to}&date=${date.toLocaleDateString("ru-RU", { day: "2-digit", month: "2-digit", year: "numeric" })}&person=${person}`);
